---
isRedeemable: false
slider:
  - "4"
  - "3"
  - "2"
projectImage: /assets/exhibita/10_thumb.webp
title: Exhibit-A
slug: exhibita
publishDate: 2023-04-15 12:00:00
collection: project
subtitle: May 2023 - Self curated - Exhibited at La Korrigane in Quebec city -
  Certificate hosted by Highlight.xyz
subtitleFr: Mai 2023 - Auto-commissariée - Exposée à La Korrigane à Québec -
  Certificat hébergé par Highlight.xyz
description: >-
  In the spring of 2023, an opportunity emerged to exhibit work at La Korrigane, a beloved pub and microbrewery in the heart of Québec City. It would be my first time showing physical work in a dedicated space — a milestone that called for something intentional. The result was Exhibit-A: a collection of 10 generative prints, conceived as a kind of personal manifesto of technique.


  The collection is built around a simple but revealing idea: five distinct algorithms, each producing two artworks as a pair. Every pair shares the same underlying foundation — Perlin noise — but diverges completely in its visual expression. Some outputs are dense and geometric, others fluid and atmospheric. Set side by side, each pair becomes a quiet argument about the range hidden inside a single mathematical concept.


  Seeing these pieces printed, framed, and hung on a wall brought a new dimension to the work. The prints were made and framed by **Les Trafiquants D’art**, collaborators since 2018, whose craftsmanship elevated each piece into something tangible and lasting. The exhibition ran from May to September 2023.


  Exhibit-A marks a turning point — the moment generative art moved off the screen and into a room, into a conversation with strangers. Certificates for the collection are hosted on Highlight.xyz, bridging the physical and the digital.
descriptionFr: >-
  Au printemps 2023, une opportunité se présente : exposer des œuvres à La Korrigane, un pub et microbrasserie bien connu du centre-ville de Québec. Ce serait la première fois que je présentais mon travail dans un espace physique dédié — un jalon qui méritait quelque chose d’intentionnel. Le résultat fut Exhibit-A : une collection de 10 impressions génératives, conçue comme une sorte de manifeste personnel de la technique.


  La collection repose sur une idée simple mais révélatrice : cinq algorithmes distincts, chacun produisant deux œuvres en paire. Chaque paire partage la même fondation — le bruit de Perlin — mais diverge complètement dans son expression visuelle. Certaines sorties sont denses et géométriques, d’autres fluides et atmosphériques. Posées côte à côte, chaque paire devient un argument silencieux sur l’étendue cachée à l’intérieur d’un seul concept mathématique.


  Voir ces pièces imprimées, encadrées et accrochées au mur a apporté une nouvelle dimension au travail. Les impressions ont été réalisées et encadrées par **Les Trafiquants D’art**, collaborateurs depuis 2018, dont le savoir-faire a élevé chaque pièce en quelque chose de tangible et durable. L’exposition a duré de mai à septembre 2023.


  Exhibit-A marque un tournant — le moment où l’art génératif a quitté l’écran pour entrer dans une salle, dans une conversation avec des inconnus. Les certificats de la collection sont hébergés sur Highlight.xyz, comme un pont entre le physique et le numérique.
url: https://www.raster.art/artwork/jb-genart-series-by-jonathan-barbeau
redeemUrl: /
image: /assets/exhibita/10_thumb.webp
images:
  - /assets/exhibita/1_thumb.webp
  - /assets/exhibita/2_thumb.webp
  - /assets/exhibita/3_thumb.webp
  - /assets/exhibita/4_thumb.webp
  - /assets/exhibita/5_thumb.webp
  - /assets/exhibita/6_thumb.webp
  - /assets/exhibita/7_thumb.webp
  - /assets/exhibita/8_thumb.webp
  - /assets/exhibita/9_thumb.webp
  - /assets/exhibita/10_thumb.webp
---
